...
public class Percolation {
    ...
    public boolean isFull(int row, int col) {
        ...
        for (int openSite : openedSites) {
            if (isTopRow(openSite) && quickFind.connected(index, openSite)) {
                return true;
            }
        }
        return false;
    ...
    public boolean percolates() {
        for (int openSite : openedSites) {
            if (isTopRow(openSite)) {
                for (int bottomSite : openedSites) {
                    if (isBottomRow(bottomSite) && quickFind.connected(openSite, bottomSite)) {
                        return true;
                    }
                }
            }
    ...
    private boolean isTopRow(int index) {
        return index < rowColSize;
    }

    private boolean isBottomRow(int index) {
        return index >= (rowColSize * (rowColSize - 1));
    }
}
